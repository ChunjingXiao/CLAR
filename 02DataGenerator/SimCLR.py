...
from losses import CustomVariationalLayer
import numpy as np
class SimCLR:
    """
        ...
        for j in range(self.num_layers_ph):
            if j < self.num_layers_ph - 1:
                ...
                self.ph_l.append(
                    ...
                    # Dense(feat_dims_ph[j])
                )

        self.SimCLR_model = self.build_model()

    def build_model(self):
        """ Building SimCLR_model,剩余num_of_unfrozen_layers层未冻结"""
    ...
    def get_callbacks(self):
        ...
        reduce_lr = ReduceLROnPlateau(
            monitor="loss", patience=5, verbose=1, factor=0.5,
        )
    ...
    def train_NL_and_evaluate(
        ...
        print("Best result from attempt:", str(results['acc']))
